...
if(!in_array($_POST['field_1'], array('name', 'domain', 'server', 'enable_website', 'webserver_ip')) || !in_array($_POST['field_2'], array('name', 'domain', 'server', 'enable_website', 'webserver_ip')))
...
if(strlen($_POST['term_1']) < 4 && $_POST['field_1'] != 'enable_website' || strlen($_POST['term_2']) < 4 && $_POST['field_2'] != 'enable_website')
	exit('<div class="alert alert-warning">Required `term` must be at least 4 characters.</div>');

if($_POST['field_1'] == 'enable_website' && !in_array($_POST['term_1'], array('0', '1')) || $_POST['field_2'] == 'enable_website' && !in_array($_POST['term_2'], array('0', '1')))
	exit('<div class="alert alert-warning">Required `term` for Has Website must be 1 or 0.</div>');
 ...
 */
	...
	$find = $mysql->prepare("SELECT * FROM `subdomains` WHERE `".$_POST['field_1']."` ".$useOperator." :term ".$middleOperator." `".$_POST['field_2']."` ".$useOperator_2." :term_2");
	$find->execute(array(
		':term' => $searchTerm,
		':term_2' => $searchTerm_2
	));
